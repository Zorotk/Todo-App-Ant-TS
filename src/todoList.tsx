        ...
        <List 
            ...
            renderItem={item => (
                <List.Item className={'list'}
                    actions={[<Button className={'description'} onClick={()=>showDescription(item.id)}>подробней</Button>,
                       <Button onClick={() => remove(item.id)}>X</Button>]}>
                    
                      <Checkbox checked={item.completed} onChange={() => toggle(item.id)} />      &nbsp;   {item.title}
                </List.Item>
            )}
            />}
            </div>
